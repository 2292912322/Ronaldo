    ...
    mounted () {
      ...
      if (true) {
        ...
        overlays.forEach(function (item) {
          BMapLib.EventWrapper.addDomListener(item._div, 'touchstart', function ({targetTouches: {[0]: {clientX, clientY}}, currentTarget: {dataset: {num}}}) {
            lastX = clientX
            lastY = clientY
          })
          BMapLib.EventWrapper.addDomListener(item._div, 'touchend', function (e) {
            let {cancelBubble, changedTouches: {[0]: {clientX, clientY}}, currentTarget: {dataset: {num}}} = e
            // 阻止拖动变点击
        ...
        let lastTouchY = 0
        // 取消选中网点
        liteMap.addEventListener('touchstart', function ({targetTouches: {[0]: {clientX, clientY}, [1]: second}}) {
          lastTouchX = clientX
          lastTouchY = clientY
          // 缩放过程中隐藏网点
          if (second) {
        ...
        liteMap.addEventListener('touchend', function ({changedTouches: {[0]: {clientX, clientY}, [1]: second}}) {
