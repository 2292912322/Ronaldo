...
export default {
  ...
  methods: {
    ...
    async onVerificationCode() {
      ...
      let { success } = await that.$api.warmcar["customer/send_verification_code"]({
        phone: that.phone,
      });
      // 发送验证码成功
      if (success) {
    ...
    async onSign() {
      ...
      let { success, bizContent, customerId } = await that.$api.warmcar["customer/login_register"]({
        phone: that.phone,
        verificationCode: that.verificationCode,
  ...
  async created() {
    ...
    if (!that.$store.state.location.cityCode) {
      var Str = "您当前未授权获取地理位置信息，无法使用租车的服务，请授权地理位置信息";
      let result = that.$wc.confirm(Str, {
        title: "温馨提示",
        confirmButtonText: "前往设置",
        cancelButtonText: "暂不租车",
        async onConfirm() {},
      });
    }
