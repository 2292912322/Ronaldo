    ...
    <div class="wc-returnCar">
      ...
      <div class="wc-card wc-returnCar-card__body">
        <i :class="imgs ? 'icons-common-selected-big-outer':'icons-common-tips-outer'">
          <div
            :class="imgs ? 'icons-common icons-common-selected-big-inner':'icons-common icons-common-tips-inner'"
          ></div>
        </i>
        <span>完成还车拍照</span>
...
export default {
  ...
  methods: {
    ...
    async returnCar() {
      ...
      let branchName = that.$store.state.returnCar.detail.branchName
        ? that.$store.state.returnCar.detail.branchName
        : that.$store.state.currentOrder.returnBranchInfo.branchName;
      if (that.current.branchName != branchName) {
        // 网点与位置不一致的错误提示
        var Str =
          ...
          async onConfirm() {
            ...
            if (that.imgs) {
              let { success, bizContent, rspCode } = await that.$api.warmcar[
                ...
                returnCarType: that.$store.state.returnCar.detail
                  .branchReturnType
                  ? that.$store.state.returnCar.detail.branchReturnType
                  : "01", //		还车方式	是	String	01 app还车	01
                //手机经纬度需在规定的如何一个网点的电子围栏范围里面才能还车
                phoneLng: that.locations.lng,//22.376689,//	手机精度
                phoneLat: that.locations.lat, //113.577893,// 手机纬度
        ...
        if (that.imgs) {
          let { success, bizContent, rspCode } = await that.$api.warmcar[
            ...
            returnCarType: that.$store.state.returnCar.detail.branchReturnType
              ...
              : "01", //		还车方式	是	String	01 app还车	01
            //手机经纬度需在规定的如何一个网点的电子围栏范围里面才能还车
            phoneLng: that.locations.lng, //22.376122, ////22.376122,		手机精度
    ...
    async loadAll() {
      ...
      let {success, bizContent} = await that.$api.warmcar['query/return_car_branch']({
        ...
        if (bizContent) { 
          let {branchInfo} = bizContent
          that.branches = that._.sortBy(branchInfo.map(o => {
            let temp = o.gps.split(',')
            let distance = 0
            if (temp.length === 2) {
              distance = that.$wc.getDistance(that.locations.lat, that.locations.lng, parseFloat(temp[1]), parseFloat(temp[0]))
            }
            return Object.assign(o, {
              distance: distance,
              num: o.leisureCarNum,
          ...
          if (!that.current.branchId) {
            ...
            // let a =that.branches.map(o =>o.branchName)
            // debugger           
          }
        }
      }
