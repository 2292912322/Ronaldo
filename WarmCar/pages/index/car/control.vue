      ...
      <div class="wc-card">
        <div class="wc-car-control__info">
          ...
          <div>
            ...
            <b></b>
            <i :class="`icons-common-battery-${restBatteryRound}-outer`">
              <div :class="`icons-common icons-common-battery-${restBatteryRound}-inner`"></div>
            </i>
            <span class="wc-car-control__info__battery"
                  :class="{'wc-car-control__info__battery--low':$store.state.currentOrder.carInfo.restBattery<=30}">{{$store.state.currentOrder.carInfo.restBattery}}%</span>
            <span class="wc-car-control__info__distance">续航{{$store.state.currentOrder.carInfo.enduranceMileage}}km</span>
          </div>
          <div v-if="$store.state.currentOrder.orderInfo.isValidate==='0'||$store.state.currentOrder.orderInfo.isValidate==='1'">
            <span class="wc-car-control__info__save">{{ $store.state.currentOrder.orderInfo.sysCancelTimeStamp >= $store.state.currentOrder.orderInfo.currentTimeStamp ? '免费保留' : '使用时长'}}：</span>
            <xy-time-count v-if="$store.state.currentOrder.orderInfo.currentTimeStamp<=$store.state.currentOrder.orderInfo.sysCancelTimeStamp"
            ...
            <xy-timing v-if="$store.state.currentOrder.orderInfo.currentTimeStamp>=$store.state.currentOrder.orderInfo.sysCancelTimeStamp"
                       class="wc-car-control__info__time"
                       :time="$store.state.currentOrder.orderInfo.takeCarTime==0 ? new Date( $store.state.currentOrder.orderInfo.sysCancelTimeStamp + $store.state.systemInfo.timePatch):new Date( $store.state.currentOrder.orderInfo.takeCarTime + $store.state.systemInfo.timePatch)">
            </xy-timing>
            <span class="wc-car-control__info__save"
                  v-if="$store.state.currentOrder.orderInfo.isValidate === '1'">行驶里程：</span>
            <span class="wc-car-control__info__long"
                  v-if="$store.state.currentOrder.orderInfo.isValidate === '1'">{{$store.state.currentOrder.orderInfo.orderMileage}}公里</span>
          </div>
        </div>
        <div class="wc-car-control__pad"
             v-show="$store.state.currentOrder.orderInfo.orderStatus==='01'|| $store.state.currentOrder.orderInfo.isValidate === '0'">
          <section>
        ...
        <div class="wc-car-control__pad wc-car-control__pad--isGet"
             v-show="$store.state.currentOrder.orderInfo.orderStatus==='05'&&$store.state.currentOrder.orderInfo.isValidate === '1'">
          <section>
            <button @click="onOpen"
                    v-xy-moving-btn>
...
<script>
  ...
  export default {
    ...
    computed:{
      restBatteryRound:function(){
       return this.$store.state.currentOrder.carInfo.restBattery<= 30 ? Math.floor((this.$store.state.currentOrder.carInfo.restBattery <= 0 ? 1 : this.$store.state.currentOrder.carInfo.restBattery) / 20) * 20 : Math.ceil((this.$store.state.currentOrder.carInfo.restBattery <= 0 ? 1 : this.$store.state.currentOrder.carInfo.restBattery) / 20) * 20
      } 
    },
    watch: {
    ...
    methods: {
      ...
      async loadAll () {
        ...
        onCancel () {
        let that = this
        if( that.$store.state.currentOrder.orderInfo.currentTimeStamp>=that.$store.state.currentOrder.orderInfo.sysCancelTimeStamp){
          that.$wc.toast('超过保留时间，无法取消预约')
        }else{
