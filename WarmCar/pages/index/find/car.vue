...
<template>
    <div class="wc-find-car" v-xy-pull-refresh="pullRefresh">
        <div class="wc-card wc-card--full">
            <div class="wc-find-car__location">
                ...
                <button @click="onNav" v-xy-moving-btn>
            ...
            <div class="wc-find-car__btns" style="display: none">
                <button>
            ...
            <div class="wc-find-car__list">
                <section v-for="item in detail.cars" :key="item.carId">
                    ...
                    <div class="wc-find-car__list__info">
                        ...
                        <div :class="`icons-common icons-common-battery-${item.restBatteryRound}-inner`"></div>
                            ...
                            <span class="wc-find-car__list__info__battery" :class="{'wc-find-car__list__info__battery--low':item.restBattery<=20}">{{item.restBattery}}%</span>
                            <span class="wc-find-car__list__info__distance">续航{{item.enduranceMileage}}km</span>
                        </div>
                        <div>
                            <span class="wc-find-car__list__info__money">{{item.timeMoney | wcNumFormat}}</span>
                            <span>元/{{item.timeUnit > 1 ? item.timeUnit : ''}}分钟+ </span>
                            <span class="wc-find-car__list__info__money">{{item.mileageMoney | wcNumFormat}}</span>
                            <span>元/{{item.mileageUnit}}</span>
                        </div>
                    </div>
                    <button @click="onCar(item)" v-xy-moving-btn>
                    用车
                  </button>
                </section>
                <xy-list-status :status="status" :bottom="2">
                    <span slot="nomore">~ 没有更多了 ~</span>
                    <div class="wc-list-empty" slot="empty">
        ...
        <actionsheet v-model="addressShow" :menus="addressSelections" @on-click-menu="onAddressClick"></actionsheet>
    </div>
</template>
                ...
                if (success && bizContent) {
                    ...
                    if (that.detail.cars && that.detail.cars.length > 0) {
                        ...
                        that.detail.cars = that.detail.cars.map(o => Object.assign(o, {
                            restBatteryRound: o.restBattery <= 30 ? Math.floor((o.restBattery <= 0 ? 1 : o.restBattery) / 20) * 20 : Math.ceil((o.restBattery <= 0 ? 1 : o.restBattery) / 20) * 20
                        }))
                    } else {
                        that.status = 'empty'
                    }
