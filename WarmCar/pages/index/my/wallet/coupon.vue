...
<template>
  <div class="wc-coupon">
    ...
    <div class="wc-coupon__content--useing" v-show="isClick&&!isUse">
    ...
    <div class="wc-coupon__content--history" v-show="!isClick&&!isUse">
    ...
    <!-- 当前订单可用优惠劵 -->
    <div class="wc-coupon__content--useing wc-coupon__content--useing--m10" v-if="isUse">
      ...
      <div class="wc-coupon__foot" @click="OnUse">
        <span>不使用优惠劵</span>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      clickNavbar($event) {
        let that = this
        that.available = $event.target.dataset.id
      ...
      OnSelect(item){
        console.log(item)
        let that = this
        that.$router.push(`../../car/return/pay?couponMny=${item.couponMoney}&orderId=${that.orderId}&couponId=${item.couponId}`)
      },
      OnUse(){
        let that = this
        that.$router.push(`../../car/return/pay?orderId=${that.orderId}`)
      },
      //查询优惠券列表
      async getCouponList(){
        let that = this
        if(that.available == '02'){
         that.isClick = false
        }else{
          that.isClick = true
        }
