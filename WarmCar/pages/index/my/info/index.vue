...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onLicense () {
        let that = this
        if (['01', '03'].includes(that.$store.state.userStatus.certificationStatus)) {
          that.$router.push('/auth/license')
        } else if (['02', '04'].includes(that.$store.state.userStatus.certificationStatus)) {
          that.$wc.toast('等待审核中')
        }
      }
    },
